...
-- Users can update their own profile, but can't change their own role unless super_admin
...
WITH CHECK (
  (id = auth.uid() AND role = OLD.role) OR  -- Users can't change their own role
  public.get_user_role(auth.uid()) IN ('admin', 'super_admin')  -- Admins can change roles
);
